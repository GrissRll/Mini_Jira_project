...
@pytest.mark.parametrize(
    ...
    [
        (
            {"status_code": 409, "detail": "User email already existing"},
            user_data_exist_email_v2,
        ),
        (
            {"status_code": 409, "detail": "User name already existing"},
            user_data_exist_name_v2,
        ),
    ],
)
...
async def test_update_router_409(
    client, create_user_fix, async_session_maker, expected, arg
):
    async with async_session_maker() as session:
...
async def test_soft_delete_200(client, async_session_maker, create_user_fix):
    ...
    access_token = create_access_token(data={"sub": user.email, "id": user.id})
    response = await client.patch(
        f"/users/change_status/1", headers={"Authorization": f"Bearer {access_token}"}
    )
    assert response.status_code == 200
    async with async_session_maker() as session:
        users_active = (
            await session.scalars(select(UserModel).where(UserModel.is_active == True))
        ).all()
...
async def test_soft_delete_403(client, async_session_maker, create_user_fix):
    ...
    access_token = create_access_token(data={"sub": user.email, "id": user.id})
    response = await client.patch(
        f"/users/change_status/222", headers={"Authorization": f"Bearer {access_token}"}
    )
    assert response.status_code == 403
    assert response.json()["detail"] == "Only owner can update or delete profile"
...
async def test_hard_delete_200(client, async_session_maker, create_user_fix):
    ...
    response = await client.delete(
        f"/users/1", headers={"Authorization": f"Bearer {access_token}"}
    )
    assert response.status_code == 200
    async with async_session_maker() as session:
        users_active = (
...
async def test_hard_delete_403(client, async_session_maker, create_user_fix):
    ...
    access_token = create_access_token(data={"sub": user.email, "id": user.id})
    response = await client.delete(
        f"/users/222", headers={"Authorization": f"Bearer {access_token}"}
    )
    assert response.status_code == 403
    assert response.json()["detail"] == "Only owner can update or delete profile"
